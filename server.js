...
app.post('/procesar', function (req, res) { //queda pending despues de ejecutar en chrome
    ...
    let id = req.body.token ? req.body.token : uuidv4();
    let pyFilePath = __dirname + `/scriptUsuario${id}.py`
    try {
        if (!fs.existsSync(pyFilePath)) {
            fs.writeFile(pyFilePath, codigoLimpio, function (err) {
                if (err) throw err;
            });

            let pyshell = new PythonShell(pyFilePath, opciones);
            Shells[id] = pyshell;
            ...
            pyshell.on('close', function (err) {
                ...
                console.log(`Ha ocurrido un error en close para scriptUsuario${id}.py: ${err}`);
                robotReady = false;
                Users.forEach((res) => {
                    res.write(`data: ${JSON.stringify({ id: -1 })} \n\n`);
                });
                pyshell = null;
                fs.unlink(pyFilePath, (err)=>{
                    if(err){
